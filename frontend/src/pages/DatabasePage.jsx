  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: COLORS.textMuted, marginBottom: 5 }}>Schema</div>
        <h2 style={{ fontSize: 24, fontWeight: 700, color: COLORS.text, margin: 0, letterSpacing: "-0.02em" }}>Database Architecture</h2>
      </div>

      {/* Tab Bar */}
      <div style={{ display: "flex", gap: 2, marginBottom: 24, background: "#F1F3FA", padding: 4, borderRadius: 12, width: "fit-content", border: "1px solid rgba(15,23,42,0.06)" }}>
        {tabs.map(([k, l]) => (
          <button key={k} onClick={() => setActiveTab(k)} style={{
            border: "none",
            background: activeTab === k ? COLORS.white : "transparent",
            color: activeTab === k ? COLORS.indigo : COLORS.textMuted,
            borderRadius: 9, padding: "7px 18px", fontSize: 12, cursor: "pointer",
            fontWeight: activeTab === k ? 700 : 500,
            letterSpacing: activeTab === k ? "0.01em" : 0,
            boxShadow: activeTab === k ? "0 1px 4px rgba(15,23,42,0.08)" : "none",
